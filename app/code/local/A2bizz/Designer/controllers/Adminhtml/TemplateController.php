...
class Sparx_Designersoftware_Adminhtml_TemplateController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		$model  = Mage::getModel('designersoftware/template')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('designersoftware/adminhtml_template_edit'))
				->_addLeft($this->getLayout()->createBlock('designersoftware/adminhtml_template_edit_tabs'));
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			$model = Mage::getModel('designersoftware/template');		
			$model->setData($data)
    ...
    public function massDeleteAction() {
        $designersoftwareIds = $this->getRequest()->getParam('template');
        if(!is_array($designersoftwareIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($designersoftwareIds as $designersoftwareId) {
                    $designersoftware = Mage::getModel('designersoftware/template')->load($designersoftwareId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($designersoftwareIds)) {
            ...
            try {
                foreach ($designersoftwareIds as $designersoftwareId) {
                    $designersoftware = Mage::getSingleton('designersoftware/template')
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('designersoftware/adminhtml_template_grid')
            ->getCsv();
