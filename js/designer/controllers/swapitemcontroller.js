customInkControllers.controller('SwapItemCtrl', ['$scope','$http','webServices','popupManager', function($scope,$http,webServices,popupManager) {
	...
	$scope.openSwapItemPopUp=function()
	{
		...
		if(!allCategory)
		{
			...
			webServices.customServerCall($http,dataAccessUrl+'designer/popup/category',obj,'swap',parScope);
		}
		else
		{
			//alert(pSubcatTitle)
	...
	$scope.openHoodiesBlanketPopUp=function()
	{
		...
		if(hoodiesOrBlanket=="Hoodies")
		{
			if(!hoodisDesignData)
			{
				...
				obj.categoryId=432
				obj.ptype="hoodies";
				webServices.customServerCall($http,dataAccessUrl+'designer/popup/productlist',obj,'hoodiesproduct',$scope)
			}
			else
			{
	...
	}	$scope.setLayerIndex=function(obj)
	{
		...
		webServices.customServerCall($http,dataAccessUrl+'designer/hoodies_index/layercolors',obj2,'layersColors',$scope)
