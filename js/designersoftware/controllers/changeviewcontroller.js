	  ...
	  // console.log(fastDeliveryTime)
	   //alert("ana")
	    ...
	    $scope.checkLicence=function()
	    {
	    	...
	    	if(editFlag=="edit" || editFlag=="template")
	    	{
	    		...
	    		webServices.customServerCall($http,dataAccessUrl+'designer/product/edit',obj,'toolEdit',$scope);
	    	}
	    	else
	    	$scope.loadTool();
	    }
	   $scope.loadTool=function()
	   {
		...
		if(hoodieFlag=='true')
		{
			...
			webServices.customServerCall($http,dataAccessUrl+'designer/hoodies_index/swapitem',obj,'toolproduct',$scope);
	    	//popupManager.close();
		}
		else
		webServices.customServerCall($http,dataAccessUrl+'designer/index/colordata',obj,'toolproduct',$scope);
	   }
	   $scope.checkLicence();
	   /********************************End licence****************************************/
	    ...
	    $scope.loadInitialColors=function()
	    {
			...
			if(!(hoodieFlag=="true"))
			{
				...
				webServices.customServerCall($http,dataAccessUrl+'designer/index/swapitem',obj,'colors',$scope);
			}
	    ...
	    $scope.loadProduct=function()
		{
			...
			webServices.customServerCall($http,dataAccessUrl+'designer/popup/productlist',obj,'product',$scope)
	    ...
	    $scope.sizes=function()
	    {
			...
			obj.productId=productId;
			//webServices.customServerCall($http,dataAccessUrl+'designer/index/swapitem',obj,'sizes',$scope);
	    ...
	    $scope.changeProductByColorId=function()
	    {
	    	...
	    	var obj=new Object();
			obj.productId=productId;
			webServices.customServerCall($http,dataAccessUrl+'designer/index/colordata',obj,'changetoolproduct',$scope);	
	    }
	    $scope.loadHoodies=function(){
	    	$scope.addObjectLoaderVariable=true;
	    	var obj=new Object();
	    	obj.productId=subProductId;
			webServices.customServerCall($http,dataAccessUrl+'designer/hoodies_index/swapitem',obj,'changetoolproduct',$scope);
	    ...
	    $scope.loadSelectProductCategory=function()
	    {
	    	...
	    	var obj=new Object();
	    	obj.productId=productId;
	    	webServices.customServerCall($http,dataAccessUrl+'designer/template/category',obj,'catselect',$scope);
	    ...
	    $scope.designNameValidation=function(str)
	    {
	    	...
	    	var obj=new Object();
	    		...
	    		webServices.customServerCall($http,dataAccessUrl+'designer/user_design/check',obj,'userDesignCheck',$scope,'',str);
	    	//$scope.sendDataToPhp()
	    }
	    	...
	    	if(angular.element('.zipCode').val())
		    	if(totalQty>0)
		    	{
		    		...
		    		if(totalQty>=selectedQuantity)
		    		{
		    			...
		    			webServices.customServerCall($http,dataAccessUrl+'designer/price',obj,'priceData',$scope,str);
	    ...
	    $scope.shareDesignToFriend=function()
	    {
	    	...
	    	webServices.customServerCall($http,dataAccessUrl+'designer/template/share',obj,'shareDesign',$scope);	
	    	else
	    	{
	    		$scope.currectInfoF=false;
	    ...
	    //alert("jhgjdh")
	    	...
	    	webServices.customServerCall($http,dataAccessUrl+'designer/template/save',obj,'saveDesign',$scope,'',saveType);	
	    	for(var i=(maintainDrawAreaArr.length-1);i>=0;i--)
			{
				maintainDrawAreaArr[i].set({opacity:1});
				canvas.renderAll();
